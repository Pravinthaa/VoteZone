...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    load();
  }, [selectedElectionId, activeTab, elections]);
      ...
      {elections.length === 0 ? (
        ...
        <div style={{ overflowY: 'auto', maxHeight: '400px' }}>
          <table className="db-table">
            <thead>
              <tr>
                <th style={{ position: 'sticky', top: 0, zIndex: 1, background: 'rgba(3,7,18,0.95)' }}>NAME</th>
                <th style={{ position: 'sticky', top: 0, zIndex: 1, background: 'rgba(3,7,18,0.95)' }}>STATUS</th>
                <th style={{ position: 'sticky', top: 0, zIndex: 1, background: 'rgba(3,7,18,0.95)' }}>POSTS</th>
                <th style={{ position: 'sticky', top: 0, zIndex: 1, background: 'rgba(3,7,18,0.95)' }}>START</th>
                <th style={{ position: 'sticky', top: 0, zIndex: 1, background: 'rgba(3,7,18,0.95)' }}>END</th>
                <th style={{ position: 'sticky', top: 0, zIndex: 1, background: 'rgba(3,7,18,0.95)' }}>ACTION</th>
              </tr>
            </thead>
            <tbody>
              {elections.map(e => {
...
{activeTab === 'candidates' && (
  <div className="db-card">
    ...
    {pendingCandidates.length === 0 ? (
      ...
      <table className="db-table">
        <thead>
          <tr>
            <th>PHOTO</th><th>STUDENT</th><th>ROLL NO</th><th>POST APPLIED</th><th>MANIFESTO</th><th>ACTION</th>
          </tr>
        </thead>
        <tbody>
            ...
            <tr key={c.id}>
              <td>
                {c.photo_path
                  ? (
                    <img
                      src={`${API_URL}/uploads/${c.photo_path}`}
                      alt={c.student_name ?? 'candidate'}
                      style={{
                        width: 36,
                        height: 36,
                        borderRadius: '50%',
                  ...
                  : <span style={{ color: 'rgba(125,211,252,0.25)', fontSize: '0.75rem' }}>—</span>
                }
              </td>
  ...
  const isActive = sel?.status === 'active';         // ← add this
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#e0f2fe', fontSize: '0.85rem' }}>
                      {idx === 0 && total > 0 && (
                        <span style={{
                          fontSize: '0.6rem',
                          background: isActive ? 'rgba(251,191,36,0.2)' : '#22c55e',  // amber when active, green when won
                          color: isActive ? '#fbbf24' : '#000',
                          border: isActive ? '1px solid rgba(251,191,36,0.4)' : 'none',
                          padding: '0.1rem 0.4rem',
                          borderRadius: '2px',
                          fontFamily: 'Rajdhani',
                          letterSpacing: '1px',
                        }}>
                          {isActive ? 'LEADING' : 'WINNER'}   {/* ← the key change */}
                        </span>
                      )}
                      {r.candidate_name}
                    </span>
